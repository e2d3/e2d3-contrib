...
for (var i=3 ; i<=data.length -1 ; i++){
    ...
    var data = [{
        ...
        id: 'SalesAttempts',
        header: [strColumn00, strColumn01, strTitle1,strColumn02]
    },{
        data: bP.partData(sales_data, 3),
        id: 'Sales',
        header: [strColumn00, strColumn01, strTitle2,strColumn03]
    },  ];
...
function iniprocess(){
	var bP={};
	var b=50, bb=100, height=500, buffMargin=1, minHeight=1,dx=50;
	...
	function drawHeader(header, id){
		d3.select("#"+id).append("g").attr("class","header").append("text").text(header[2])
			.style("font-size","20").attr("x",100).attr("y",-20).style("text-anchor","middle")
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text(header[3]).attr("x", (c2[d]-25))
				.attr("y", -5).style("fill","grey");
