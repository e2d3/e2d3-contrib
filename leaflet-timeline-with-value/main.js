...
function show(data)
{
  ...
  var valueScale = d3.scale.linear().range([0, valueMaxR]);
  ...
  var startAnim = function(t0, dt) {
    ...
    tick = function() {
      ...
      }).attr('opacity', 0.8).attr({
        r:function(d){
          return valueScale(d.value)
        }
      }).transition().delay(0).attr('opacity', 0.0);
  ...
  graphLayer.append('rect')
    ...
    .on('click', function() {
      ...
      startAnim(graphXScale.invert(d3.event.x - margin.left - padding.left), (timeScale.domain()[1].getTime() - timeScale.domain()[0].getTime())/speed);
    }).call(drag);
