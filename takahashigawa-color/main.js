...
var projection = d3.geo.mercator()
  .center([133.32,34.78])
  .scale(Math.min(width, height) * 50)
...
var placeName = 'takahashiriver';

var topoSelection = null;

d3.json(baseUrl + "/takahashiriver.topojson", function(error, o) {
  ...
  d3.json(baseUrl + "/river.topojson", function(error, oo)
  {
    ...
    riverLayer.selectAll('.river')
      ...
      .on('mouseover', function(d)
      {
        ...
        tooltipTitle.text(d.properties.river);
...
function show(data)
{
  if (data && topo.objects)
  {
    ...
    {
      d.properties.data = {};
      d.properties.data = data[d.properties.city] || {};
    });
    ...
    topoSelection.on('mouseover', function(d)
    {
      tooltip.style('display','block');
      tooltip.attr('transform','translate('+d3.event.pageX+','+(d3.event.pageY+40)+')');
      tooltipTitle.text(d.properties.prefecture+d.properties.city);
      tooltipData.selectAll('text').remove();
